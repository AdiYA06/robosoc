...
def fetch_state(endpoint: str, api_token: str, timeout_s: float) -> tuple[bool, ControlState | None, float]:
...
def main() -> None:
    ...
    try:
        while True:
            start = time.time()
            ok, fetched_state, remote_age = fetch_state(args.endpoint, args.token, args.http_timeout_s)
            now = time.time()
